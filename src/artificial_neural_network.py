...
class ArtificialNeuralNetworkRecognizer:
    def __init__(self) -> None:
        self._name = "Neural Network Classifier"
        self._nn = nn.MLPClassifier(solver="adam", hidden_layer_sizes=(256, 64, 10), max_iter=300)
    ...
    def train(self, train_data, train_labels):
        ...
        self._nn.fit(self._split_train_data , self._split_train_label)
        ...
        if (_HYPERPARAMETER_TURNING):
            ...
            hyper_parameters = [{"hidden_layer_sizes": [(256, 64, 10), (512, 128, 10)], "max_iter": [300, 500]}]
            boost_model = nn.MLPClassifier(solver="adam")
            ...
            cv_results["param_max_iter"] = cv_results["param_max_iter"].astype(int)

            _, axes = plt.subplots(1, 2, figsize=(20, 5))

            axes[0].set_title("Hidden layer size of (256, 64, 10)")
            axes[0].set_xlabel("Number of estimators")
            axes[0].set_ylabel("Accuracy")
            axes[0].grid()
            ls50 = cv_results[cv_results["param_hidden_layer_sizes"] == (256, 64, 10)]

            axes[0].plot(ls50["param_max_iter"], ls50["mean_test_score"])
            axes[0].plot(ls50["param_max_iter"], ls50["mean_train_score"])
            axes[0].set_ylim(0.5, 1)
            axes[0].legend(["Test Accuracy", "Train Accuracy"], loc='upper left')

            #

            axes[1].set_title("Hidden layer size of (512, 128, 10)")
            axes[1].set_xlabel("Number of estimators")
            axes[1].set_ylabel("Accuracy")
            axes[1].grid()
            ls100 = cv_results[cv_results["param_hidden_layer_sizes"] == (512, 128, 10)]

            axes[1].plot(ls100["param_max_iter"], ls100["mean_test_score"])
            axes[1].plot(ls100["param_max_iter"], ls100["mean_train_score"])
            axes[1].set_ylim(0.5, 1)
            axes[1].legend(["Test Accuracy", "Train Accuracy"], loc='upper left')
         
            plt.savefig("Neural Net Tuning.png")
    ...
    def predict(self, test_data, test_labels): 
        val_score = self._nn.score(self._split_val_data, self._split_val_label)

        start_test_time = time.time()
    
        test_score = self._nn.score(test_data, test_labels)
