...
class KNearestNeighboursRecognizer:
    def __init__(self) -> None:
        self._name = "KNN Classifier"

        self._knn = neighbors.KNeighborsClassifier(n_neighbors = 5, weights="uniform")
    ...
    def train(self, train_data, train_labels):
        ...
        self._knn.fit(self._split_train_data , self._split_train_label)
        ...
        if (_HYPERPARAMETER_TURNING):
            ...
            hyper_parameters = [{"n_neighbors": [5, 10, 15], "weights": ["uniform", "distance"]}]

            knn_model = neighbors.KNeighborsClassifier()

            # Grid search
            model_cv = GridSearchCV(estimator=knn_model, param_grid=hyper_parameters, scoring="accuracy", cv=folds, verbose=1, return_train_score=True)
            ...
            axes[0].grid()
            kd_tree = cv_results[cv_results["param_weights"] == "uniform"]

            axes[0].plot(kd_tree["param_n_neighbors"], kd_tree["mean_test_score"])
            axes[0].plot(kd_tree["param_n_neighbors"], kd_tree["mean_train_score"])
            axes[0].set_ylim(0.5, 1)
            axes[0].legend(["Test Accuracy", "Train Accuracy"], loc='upper left')

            #
            axes[1].set_title("Distance based weights")
            axes[1].set_xlabel("Number of neighbours")
            axes[1].set_ylabel("Accuracy")
            axes[1].grid()
            balltree = cv_results[cv_results["param_weights"] == "distance"]

            axes[1].plot(balltree["param_n_neighbors"], balltree["mean_test_score"])
            axes[1].plot(balltree["param_n_neighbors"], balltree["mean_train_score"])
            axes[1].set_ylim(0.5, 1)
            axes[1].legend(["Test Accuracy", "Train Accuracy"], loc='upper left')

            plt.savefig("KNN Tuning.png")
    ...
    def predict(self, test_data, test_labels): 
        val_score = self._knn.score(self._split_val_data, self._split_val_label)

        start_test_time = time.time()
    
        test_score = self._knn.score(test_data, test_labels)
